...
sys.path.append(os.path.join(os.path.dirname(__file__), 'sp_tool'))
...
def find_all_subfolder_prefixes_and_input_files(args):
    ...
    # return res
    """
    Extract a matching set of paths to "1DCNNBLSTM.arff" files located within a "miccai2025" folder under each subdirectory
    of the specified rootpath.
    :param rootpath: base directory to search in
    :return: a list of tuples, where the first element is the sub-folder prefix and the second one is the full path
            to each "1DCNNBLSTM.arff" file
    """
    res = []
    # List all subdirectories in the provided rootpath
    folders = [name for name in os.listdir(args.input) if os.path.isdir(os.path.join(args.input, name))]
    
    for folder in folders:
        miccai_dir = os.path.join(args.input, folder, 'benchmarks')
        if os.path.isdir(miccai_dir):
            arff_file = os.path.join(miccai_dir, 'raw_1DCNNBLSTM.arff')
            if os.path.isfile(arff_file):
                subfolder_prefix = os.path.join(folder, 'benchmarks')  # Adjust if you need a different subfolder structure
                res.append((subfolder_prefix, arff_file))
...
def get_corresponding_output_paths(subfolders_and_full_input_filenames, args):
    ...
    # return res
    """
    Create a list of output paths where each output file will have the same path as the input file but with a
    different file name ("1DCNNBLSTM_out.arff") in the corresponding 'benchmarks' subfolder.
    :param subfolders_and_full_input_filenames: subfolder prefixes and full input file names
           returned by find_all_subfolder_prefixes_and_input_files()
    :param args: command line arguments
    :return: list of output file paths
    """
    # Set the base output directory to the input directory if not specified
    if args.output is None:
        args.output = os.path.dirname(subfolders_and_full_input_filenames[0][1])
        print('No --output was provided, using the input directory:', args.output, file=sys.stderr)

    res = []
    for subfolder, full_input_name in subfolders_and_full_input_filenames:
        # Construct the directory path using the subfolder prefix
        output_dir = os.path.join(args.output, subfolder)
        # Ensure the output directory exists
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        # Construct the full output file path with the new filename
        output_file = os.path.join(output_dir, '1DCNNBLSTM_pred_points.arff')
